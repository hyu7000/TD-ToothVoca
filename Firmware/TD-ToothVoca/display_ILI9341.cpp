...
void printText(uint16_t x, uint16_t y, char *text, uint8_t align, uint16_t color)
{
  ...
  while(index < text_len)
  {
    ...
    {
      drawText(NON_KOREAN, align, &xIndex, &yIndex, &text[index], 0,     x, TFT_WIDTH-X_END_POS_IN_ALIGN_CENTER, color);
      index++;
    }
    else if(isKoreanText(text, index))
    {
      drawText(KOREAN,     align, &xIndex, &yIndex, text,         index, x, TFT_WIDTH-X_END_POS_IN_ALIGN_CENTER, color);
      index += 3;
    }
    else
    {
      drawText(NON_KOREAN, align, &xIndex, &yIndex, "?",          0,     x, TFT_WIDTH-X_END_POS_IN_ALIGN_CENTER, color);
      index++;
    }
  }
}
...
void calCenterPosOfText(char *text)
{
  ...
  if(widthOfAllText < (TFT_WIDTH - X_START_POS_IN_ALIGN_CENTER*2))
  {
    textStartEndPos.start_X_Pos = (TFT_WIDTH - widthOfAllText)/2;
    textStartEndPos.numOfLine   = 0;
  }
  else if(widthOfAllText > (TFT_WIDTH - X_START_POS_IN_ALIGN_CENTER*2))
  {
    uint8_t lines = widthOfAllText / (TFT_WIDTH - X_START_POS_IN_ALIGN_CENTER*2);
